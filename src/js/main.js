...
const closeVideosOverlay = document.querySelectorAll('.popup-overlay');
...
closeVideosOverlay.forEach((close) => {
...
// 		const dataSubmit = event.data.data;
// 		console.log("Form Submitted!");
// 		// let text = event.data.data[0].value;
// 		const inputHiddenValue = document.querySelector('.inputArr');
// 	    const dataValue = [];
// 		for (let i = 0; i < dataSubmit.length; i++) {
// 			var obj = {};
// 			for (let i = 0; i < dataSubmit.length; i++) {
// 				obj[dataSubmit[i].name] = dataSubmit[i].value;
// 			}
// 			dataValue.push(obj);
// 			inputHiddenValue.value = obj;
// 		}
// 		console.log(obj);
// 		localStorage.setItem("dataSubmitItem", inputHiddenValue.value);
// 	}
// });
// const inputHiddenValue = document.querySelector('.inputArr');
// console.log(inputHiddenValue.value);
//
// window.addEventListener('message', event => {
// 	if (
...
// 		console.log('event.data.value'+ event.data[2].value);
//
// 	}
// });
// window.addEventListener("message", function(event) {
...
const singleEditor = document.querySelector('.section-blog__item');
if (singleEditor) {
    ...
    if (window.screen.width > 1023) {
        ...
        (function feauterScrollColor() {
            ...
            let elementsPage = document.querySelectorAll('.text-entry__blog>h2');
            scrollerColors(elementsPage, singleEditor)
        }());
    }
}
...
if (document.querySelector('.menu-item-has-children')) {
	...
	if($(window).width() < 992 ){
		...
		$navLink.each(function (e) {if($(this).next().length > 0){$(this).addClass('hideClick');}});
		$navLink.on('click', function (e) {
			if($(this).hasClass('hideClick')){
				e.preventDefault();
		...
		const $navFotLink = $('.page-footer h4');
		$navFotLink.each(function (e) {if($(this).next().length > 0){$(this).addClass('hideClick');}});
		$navFotLink.on('click', function (e) {
			if($(this).hasClass('hideClick')){
				e.preventDefault();
				if($(this).parent().hasClass('active')){
					$(this).parent().removeClass('active');
					...
					$('.page-footer h4').removeClass('active');
					$(this).parent().addClass('active');
					$(this).next().addClass('active');
				}
			}
	...
	var addClassOnScroll = function () {
		...
		videoData.each(function(){
			if($(this).attr('data-src')){
				if($(this).parent().hasClass('video-banner-background')){
		...
		imageData.each(function () {
			if ($(this).attr('data-src')) {
				if($(this).parent().parent().hasClass('text-banner-background')){
	...
	if($('.text-banner-background').length > 0){
		const imageDataSrc = $('.text-banner-background').find('img').attr('data-src');
		$('.text-banner-background').find('img').attr('src', imageDataSrc);
		$('.text-banner-background').find('img').removeAttr('data-src');
	}
